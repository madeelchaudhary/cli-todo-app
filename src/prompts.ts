...
async function askTask(availableTasks: string[]) {
  const answers = await inquirer.prompt({
    name: "userChoice",
    type: "list",
...
function validteTodoFields(this: any, input: string) {
  if (!input) {
...
async function askSelectTodo(customMessage: string, todos: Todo[]) {
  logNote(
...
async function askSelectTodoCategory(
