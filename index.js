#! /usr/bin/env node
import { askName, askSelectTodo, askSelectTodoCategory, askTask, askToContinue, askTodo, } from "./src/prompts.js";
import Todo, { categories } from "./src/todo.js";
import { closingMessage, logError, logNote, logTodos, welcome, } from "./src/logs.js";
var AvailableTasks;
(function (AvailableTasks) {
    AvailableTasks[AvailableTasks["VIEW"] = 0] = "VIEW";
    AvailableTasks[AvailableTasks["VIEW_BY_CATEGORY"] = 1] = "VIEW_BY_CATEGORY";
    AvailableTasks[AvailableTasks["ADD"] = 2] = "ADD";
    AvailableTasks[AvailableTasks["UPDATE"] = 3] = "UPDATE";
    AvailableTasks[AvailableTasks["DELETE"] = 4] = "DELETE";
})(AvailableTasks || (AvailableTasks = {}));
let userName;
async function init() {
    do {
        try {
            const userChosenTaskStr = await askTask(Object.keys(AvailableTasks).slice(5));
            ...
            if (userChosenTask === AvailableTasks.ADD) {
                ...
                const todo = new Todo(userEnteredTodo.title, userEnteredTodo.description, userEnteredTodo.category);
                todo.save();
                ...
                const userChosenTodoId = await askSelectTodo("Select todo you want to update.", todos);
                const userTodoEntries = await askTodo();
                const updatedTodo = new Todo(userTodoEntries.title, userTodoEntries.description, userTodoEntries.category);
                ...
                const userChosenTodoId = await askSelectTodo("Select todo you want to delete.", todos);
                ...
                const userChosenCategory = await askSelectTodoCategory("Select the category.", allCategories);
